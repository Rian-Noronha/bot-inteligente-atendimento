const { ChatConsulta, ChatResposta, ChatSessao, AssuntoPendente, Feedback, Documento, Subcategoria, Categoria, sequelize} = require('../models');
...
exports.getKpis = async (req, res) => {
    ...
    const [
        ...
        ChatSessao.findOne({
            ...
            where: { registro_fim: { [Op.not]: null }, ...filtroDeSessao }
        }),
        ChatResposta.count({
            where: {
    ...
    const totalSessoes = await ChatSessao.count({ where: filtroDeSessao });
